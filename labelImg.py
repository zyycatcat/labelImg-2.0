...
from libs.label_template import get_tag_cood_tuple_list
...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None):
        ...
        self.autoLabelButton.setToolButtonStyle(Qt.ToolButtonTextBesideIcon)

        # 滑块，用于调节模板匹配阈值
        self.slider = QSlider(Qt.Horizontal, self)
        self.slider.setFixedSize(200, 20)
        self.slider.setMinimum(50)      # 最小0.5
        self.slider.setMaximum(100)     # 最大1.0
        self.slider.setValue(80)
        self.slider.valueChanged[int].connect(self.sliderEvent)
        # 用于展示阈值的标签
    ...
    def autoLabel(self):
        ...
        for i in range(self.labelList.count()):
            ...
            for point in shape.points:
                ...
                if point.y() > ymax:
                    ...
                    ymin = point.y()
            all_tag_list.append([shape.label, [int(xmin), int(ymin), int(xmax), int(ymax)], shape.difficult])
            if shape.difficult:
        ...
        for new_tag in tag_cood_tuple_list:
            ...
            new_shape.addPoint(QPointF(x_min, y_min))
            new_shape.addPoint(QPointF(x_max, y_min))
            new_shape.addPoint(QPointF(x_max, y_max))
            new_shape.addPoint(QPointF(x_min, y_max))
            self.addLabel(new_shape)
            shapes.append(new_shape)
        self.canvas.loadShapes(shapes)
        self.setDirty()
        self.autoLabelButton.setEnabled(True)
        print('debug# match template done!')
